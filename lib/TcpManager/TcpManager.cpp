...
void TcpManager::ensureClientConnected() {
  ...
  if (now - lastReconnectAttempt_ < 3000UL) {
    return;
  }
  ...
  client_.stop();
  if (client_.connect(ip, settings_.serverPort)) {
...
bool TcpManager::hasClient() {
  return incomingClient_.connected();
}

String TcpManager::lastMessage() const {
  return lastMessage_;
}