...
DeviceConfig ConfigManager::load() {
    ...
    config_.tcp.autoReconnect = prefs_.getBool("tcp_recon", config_.tcp.autoReconnect);
    config_.tcp.reconnectIntervalMs = prefs_.getUShort("tcp_rint", config_.tcp.reconnectIntervalMs);
    config_.tcp.connectTimeoutMs = prefs_.getUShort("tcp_cto", config_.tcp.connectTimeoutMs);
    ...
    config_.scaleTcp.autoReconnect = prefs_.getBool("sc_recon", config_.scaleTcp.autoReconnect);
    config_.scaleTcp.reconnectIntervalMs = prefs_.getUShort("sc_rint", config_.scaleTcp.reconnectIntervalMs);
    config_.scaleTcp.connectTimeoutMs = prefs_.getUShort("sc_cto", config_.scaleTcp.connectTimeoutMs);
...
bool ConfigManager::save(const DeviceConfig& config) {
    ...
    prefs_.putBool("tcp_recon", config.tcp.autoReconnect);
    prefs_.putUShort("tcp_rint", config.tcp.reconnectIntervalMs);
    prefs_.putUShort("tcp_cto", config.tcp.connectTimeoutMs);
    ...
    prefs_.putBool("sc_recon", config.scaleTcp.autoReconnect);
    prefs_.putUShort("sc_rint", config.scaleTcp.reconnectIntervalMs);
    prefs_.putUShort("sc_cto", config.scaleTcp.connectTimeoutMs);
...
IPAddress ConfigManager::stringToIp(const String& ipStr) {
    IPAddress ip;
    if (!ip.fromString(ipStr)) return IPAddress(0, 0, 0, 0);
    return ip;
}
