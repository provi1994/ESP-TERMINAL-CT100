    ...
    server_.on(
        ...
        [this]() {
            ...
            server_.send(
                ...
                ok
                    ? "<h3>Firmware upload OK</h3><p>Urządzenie uruchomi się ponownie.</p><p><a href='/'>Powrot</a></p>"
...
void WebConfigServer::handleFirmwarePage() {
    ...
    html += "<div class='sub'>Rola: <strong>" + currentRoleName() + "</strong> | Uzytkownik: <strong>" +
            jsonEscape(currentUserName()) + "</strong></div></section>";
    html += "<section><h2>Upload pliku .bin</h2>";
    html += "<div class='sub'>Serwisant moze wgrac nowe oprogramowanie bez dostepu do zmiany hasel. Po poprawnym uploadzie urzadzenie zrestartuje sie automatycznie.</div>";
...
<div class="app">
  <aside class="sidebar">
    ...
    <div class="brand-sub">Panel serwisowy. Najpierw integracja, stabilnosc i diagnostyka. Bez zbednych live dashboardow.</div>
    <nav class="nav">
      <a href="#start">Start</a><a href="#network">Siec</a><a href="#integration">Integracja</a><a href="#io">RFID i wejscia</a><a href="#frames">Ostatnie ramki</a><a href="#logs">Logi i diagnostyka</a><a href="#firmware">Firmware / OTA</a><a href="#admin" id="navAdmin">Administracja</a>
    </nav>
    <div class="rolebox"><div><strong>Rola:</strong> %CURRENT_ROLE%</div><div><strong>Uzytkownik:</strong> %CURRENT_USER%</div><div style="margin-top:8px">service: montaz, IP, porty, testy, logi, OTA</div><div>admin: dodatkowo hasla i ustawienia krytyczne</div></div>
  </aside>
  <main class="main">
    <section class="section" id="start"><h1>Start</h1><div class="sub">To jest panel dla serwisanta i integratora. Najpierw podstawowy stan urzadzenia, potem konfiguracja i diagnostyka.</div><div class="grid3 mt"><div class="card"><div class="label">Urzadzenie</div><div class="value" id="deviceNameHead">%DEVICE_NAME%</div></div><div class="card"><div class="label">Rola</div><div class="value" id="currentRole">%CURRENT_ROLE%</div></div><div class="card"><div class="label">Wersja</div><div class="value" id="fwVersion">0.1.0</div></div></div><div class="grid3 mt"><div class="card"><div class="label">IP</div><div class="value" id="liveIp">-</div></div><div class="card"><div class="label">TCP komendy</div><div class="value" id="cmdState">-</div></div><div class="card"><div class="label">TCP waga</div><div class="value" id="scaleState">-</div></div></div><div class="grid3 mt"><div class="card"><div class="label">RFID</div><div class="value" id="rfidState">-</div></div><div class="card"><div class="label">Klawiatura</div><div class="value" id="keypadState">-</div></div><div class="card"><div class="label">Discovery</div><div class="value" id="discState">-</div></div></div><div class="actions"><a class="btn btn-blue" href="#network">Konfiguracja sieci</a><a class="btn btn-blue" href="#integration">Integracja TCP</a><a class="btn btn-gray" href="/logs">Pokaz logi</a><a class="btn btn-blue" href="/firmware">OTA</a></div></section>
    <section class="section" id="network"><h2>Siec</h2><div class="sub">Ustawienia podstawowe do uruchomienia terminala w sieci klienta.</div><div class="row mt"><div><label class="label">Nazwa urzadzenia</label><input id="deviceName" type="text"></div><div><label class="label">Tryb sieci</label><select id="networkMode"><option value="dhcp">DHCP</option><option value="static">Static</option></select></div></div><div class="row mt"><div><label class="label">IP</label><input id="ip" type="text"></div><div><label class="label">Brama</label><input id="gateway" type="text"></div></div><div class="row mt"><div><label class="label">Maska</label><input id="subnet" type="text"></div><div><label class="label">DNS 1</label><input id="dns1" type="text"></div></div><div class="row mt"><div><label class="label">DNS 2</label><input id="dns2" type="text"></div><div class="card"><div class="label">Aktualny IP</div><div class="value" id="networkIpNow">-</div><div class="label" style="margin-top:12px">MAC</div><div class="value" id="macNow">-</div></div></div></section>
    <section class="section" id="integration"><h2>Integracja</h2><div class="sub">Ustawienia komunikacji z programem wagowym i ewentualnym kanalem wagi ASCII.</div><div class="grid"><div class="card"><h3>TCP komendy</h3><div class="row"><div><label class="label">Tryb</label><select id="tcpMode"><option value="client">client</option><option value="host">host</option><option value="server">server</option></select></div><div><label class="label">Port serwera</label><input id="tcpServerPort" type="number"></div></div><div class="row mt"><div><label class="label">IP serwera</label><input id="tcpServerIp" type="text"></div><div><label class="label">Port nasluchu</label><input id="tcpListenPort" type="number"></div></div></div><div class="card"><h3>TCP waga ASCII</h3><label class="check"><input id="scaleEnabled" type="checkbox"> Wlacz kanal wagi</label><div class="row mt"><div><label class="label">Tryb</label><select id="scaleMode"><option value="client">client</option><option value="host">host</option><option value="server">server</option></select></div><div><label class="label">Port serwera</label><input id="scaleServerPort" type="number"></div></div><div class="row mt"><div><label class="label">IP serwera</label><input id="scaleServerIp" type="text"></div><div><label class="label">Port nasluchu</label><input id="scaleListenPort" type="number"></div></div></div></div></section>
    <section class="section" id="io"><h2>RFID i wejscia</h2><div class="sub">Skonfiguruj lokalne wejscia i sprawdz, jakie ramki generuje terminal.</div><div class="grid"><div class="card"><h3>RFID</h3><label class="check"><input id="rfidEnabled" type="checkbox"> Wlacz RFID</label><div class="row mt"><div><label class="label">Baud rate</label><input id="rfidBaud" type="number"></div><div><label class="label">Kodowanie</label><select id="rfidEncoding"><option value="hex">HEX</option><option value="dec">DEC</option><option value="raw">RAW</option><option value="scale_frame">CT100 frame</option></select></div></div><div class="mt note">Ostatni UID: <strong id="lastRfidValue">-</strong></div></div><div class="card"><h3>Klawiatura i discovery</h3><label class="check"><input id="keypadEnabled" type="checkbox"> Wlacz klawiature</label><div class="row mt"><div><label class="label">Adres PCF8574</label><input id="pcf8574Address" type="number"></div><div><label class="label">Ostatni klawisz</label><input id="lastKeyValue" type="text" disabled></div></div><label class="check mt"><input id="discoveryEnabled" type="checkbox"> Wlacz discovery UDP</label><div class="mt"><label class="label">Port discovery</label><input id="udpPort" type="number"></div></div></div></section>
    <section class="section" id="frames"><h2>Ostatnie ramki</h2><div class="sub">To jest glowny podglad diagnostyczny. Zamiast ciezkiego live dashboardu pokazujemy ostatnia komunikacje i slad transmisji.</div><div class="grid mt"><div class="card"><div class="label">Ostatnia ramka wychodzaca</div><div class="value" id="lastOutbound">-</div></div><div class="card"><div class="label">Ostatnia ramka przychodzaca</div><div class="value" id="lastInbound">-</div></div></div><div class="grid mt"><div class="card"><div class="label">Ostatnia wiadomosc TCP komendy</div><div class="value" id="cmdLastMsg">-</div></div><div class="card"><div class="label">Ostatnia wiadomosc TCP waga</div><div class="value" id="scaleLastMsg">-</div></div></div><div class="actions"><a class="btn btn-gray" href="/status" target="_blank">Status TXT</a><a class="btn btn-gray" href="/logs" target="_blank">Logi</a></div></section>
    <section class="section" id="logs"><h2>Logi i diagnostyka</h2><div class="sub">Logi tylko na zadanie, bez agresywnego odswiezania. To jest lzejsze i bezpieczniejsze dla ESP.</div><div class="actions"><a class="btn btn-gray" href="/logs" target="_blank">Otworz logi</a><a class="btn btn-gray" href="/status" target="_blank">Otworz status TXT</a></div></section>
    <section class="section" id="firmware"><h2>Firmware / OTA</h2><div class="sub">Aktualizacja firmware jest dostepna dla service i admin. Zmiana hasel OTA pozostaje tylko dla admina.</div><div class="actions"><a class="btn btn-blue" href="/firmware">Przejdz do uploadu .bin</a></div></section>
    <section class="section" id="admin"><h2>Administracja</h2><div class="sub">Ta sekcja jest wylacznie dla admina. Service nie ma tu dostepu do zmiany hasel.</div><div id="adminVisibleBlock" class="hidden"><div class="row mt"><div><label class="label">Admin user</label><input id="adminUser" type="text"></div><div><label class="label">Admin haslo</label><input id="adminPassword" type="password" placeholder="Podaj nowe lub zostaw bez zmian"></div></div><div class="row mt"><div><label class="label">Service user</label><input id="serviceUser" type="text"></div><div><label class="label">Service haslo</label><input id="servicePassword" type="password" placeholder="Podaj nowe lub zostaw bez zmian"></div></div><div class="row mt"><div><label class="label">Haslo OTA</label><input id="otaPassword" type="password" placeholder="Podaj nowe lub zostaw bez zmian"></div><div class="card"><div class="label">Ostrzezenie</div><div class="note">Hasla zapisują się tylko po kliknięciu Zapisz konfigurację. Zmiany wymagają restartu urządzenia.</div></div></div></div><div id="adminHiddenBlock" class="card"><div class="label">Brak dostepu</div><div class="note">Zaloguj sie jako admin, aby zarzadzac haslami i ustawieniami krytycznymi.</div></div></section>
    <section class="section"><h2>Akcje serwisowe</h2><div class="actions"><button class="btn btn-blue" id="saveBtn" type="button">Zapisz konfiguracje</button><button class="btn btn-orange" id="saveRestartBtn" type="button">Zapisz i restart</button><button class="btn btn-gray" id="reloadBtn" type="button">Odswiez dane</button><form method="post" action="/reboot" style="margin:0"><button class="btn btn-orange" type="submit">Restart teraz</button></form></div><div class="mt note" id="saveMsg">Zmiany konfiguracji zapisują się przez API i wymagają restartu urządzenia.</div></section>
  </main>
</div>
<script>
(function(){
  ...
  async function loadRuntime(){ if(inflightRuntime) return; inflightRuntime = true; try{ const data = await fetchJson("/api/runtime"); setText("cmdState", data.cmdTcpConnected ? "polaczono" : "brak polaczenia"); setText("scaleState", data.scaleTcpConnected ? "polaczono" : "brak polaczenia"); setText("rfidState", data.rfidEnabled ? "aktywne" : "wylaczone"); setText("keypadState", data.keypadEnabled ? (data.keypadDetected ? "aktywna" : "wlaczona, brak odpowiedzi") : "wylaczona"); setText("discState", data.discoveryEnabled ? "aktywne" : "wylaczone"); setText("lastRfidValue", data.rfidLast); setText("lastKeyValue", data.keyLast); setText("lastOutbound", data.lastOutbound); setText("lastInbound", data.lastInbound); setText("cmdLastMsg", data.cmdTcpLast); setText("scaleLastMsg", data.scaleTcpLast); if(data.ip) { setText("liveIp", data.ip); setText("networkIpNow", data.ip); } } finally { inflightRuntime = false; } }
  ...
  async function saveConfig(){ disableButtons(true); setText("saveMsg", "Zapisywanie konfiguracji..."); try{ const payload = collectPayload(); const res = await fetch("/api/config", { method:"POST", headers:{"Content-Type":"application/json"}, body: JSON.stringify(payload) }); if(!res.ok) throw new Error("HTTP " + res.status); const data = await res.json(); setText("saveMsg", data.restartRequired ? "Konfiguracja zapisana. Wymagany restart urzadzenia." : "Konfiguracja zapisana."); if(restartAfterSave){ restartAfterSave = false; setTimeout(() => { const form = document.createElement("form"); form.method = "POST"; form.action = "/reboot"; document.body.appendChild(form); form.submit(); }, 300); } } catch(err){ setText("saveMsg", "Blad zapisu: " + err.message); restartAfterSave = false; } finally { disableButtons(false); } }
  ...
  el("reloadBtn").addEventListener("click", async () => { disableButtons(true); try{ await Promise.all([loadDeviceInfo(), loadConfig(), loadRuntime()]); setText("saveMsg", "Dane odswiezone."); } catch(err){ setText("saveMsg", "Blad odswiezania: " + err.message); } finally { disableButtons(false); } });
  if(role !== "admin"){ el("navAdmin").classList.add("hidden"); }
  Promise.all([loadDeviceInfo(), loadConfig(), loadRuntime()]).catch(err => setText("saveMsg", "Blad startu panelu: " + err.message));
